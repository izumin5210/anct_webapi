...
describe "Api::V1::Lectures" do
  ...
  describe 'GET /api/v1/lectures/:id' do
    ...
    before(:each) { get "api/v1/lectures/#{lecture.id}" }
    ...
    it 'returns lecture json', autodoc: true do
      ...
      expect(response.body).to be_json_eql("\"#{lecture.required_selective}\"").at_path('lecture/requiredSelective')
      ...
      expect(response.body).to be_json_eql("\"#{lecture.failure_absence}\"").at_path('lecture/failureAbsence')
      ...
      expect(response.body).to be_json_eql(lecture.year).at_path('lecture/year')
